...
public partial class Admin_Proforma : System.Web.UI.Page
{
    ...
    //{
    //    SqlDataAdapter ad = new SqlDataAdapter("SELECT DISTINCT [AgainstNo] FROM [tbl_Proforma_Hdr] ", con);
    //    DataTable dt = new DataTable();
    //    ad.Fill(dt);
    //    if (dt.Rows.Count > 0)
    //    {
    ...
    protected void GenerateInvoice()
    {
        ...
        string query = $"SELECT MAX([Id]) AS maxid FROM [tbl_Proforma_Hdr] WHERE YEAR([InvoiceDate]) IN ({currentYear - 1}, {currentYear})";
    ...
    //    // Construct the SQL query to get the maximum invoice ID for the current year
    //    string query = $"SELECT MAX([Id]) AS maxid FROM [tbl_Proforma_Hdr] WHERE YEAR([InvoiceDate]) = {currentYear}";
    ...
    //protected void GenerateInvoice()
    //{
    //    SqlDataAdapter ad = new SqlDataAdapter("SELECT max([Id]) as maxid FROM [tbl_Proforma_Hdr]", con);
    //    DataTable dt = new DataTable();
    //    ad.Fill(dt);
    //    if (dt.Rows.Count > 0)
    //    {
    ...
    {
        DataTable Dt = new DataTable();
        SqlDataAdapter da = new SqlDataAdapter("SELECT [tbl_Proforma_Dtls].JobNo, [tbl_Proforma_Dtls].MateName,[tbl_Proforma_Dtls].PrintDescription,tbl_Proforma_Hdr.InvoiceNo,tbl_Proforma_Hdr.InvoiceAgainst,tbl_Proforma_Hdr.AgainstNo,InvoiceDate,tbl_Proforma_Hdr.JobNo,tbl_Proforma_Hdr.CompName,tbl_Proforma_Hdr.CustName,PoNo,PoDate,ChallanNo,ChallanDate,PayTerm,Delivery,KindAtt,CompanyAddress,CompanyGstNo,CompanyPanNo, ComapyRegType, CompanyStateCode, CustomerShippingAddress, CustomerGstNo,Term_Condition_1,Term_Condition_2,Term_Condition_3,Term_Condition_4,Term_Condition_5,Term_Condition_6, CustomerPanNo, CustomerRegType,CustomerStateCode, CGST, SGST,IGST, AllTotalAmount, GrandTotal, TotalInWord, Description, Hsn, TaxPercentage, Quntity, Unit, Rate, DiscountPercentage, Total FROM tbl_Proforma_Hdr INNER JOIN [tbl_Proforma_Dtls] ON [tbl_Proforma_Dtls].InvoiceId = tbl_Proforma_Hdr.Id WHERE tbl_Proforma_Hdr.Id='" + hdnID.Value + "'", con);
        da.Fill(Dt);
        ...
        DataTable Dt_Product = new DataTable();
        SqlDataAdapter daa = new SqlDataAdapter("SELECT MateName,Description,Hsn,Rate,Unit,Quntity,TaxPercentage,DiscountPercentage,Total FROM [tbl_Proforma_Dtls] WHERE tbl_Proforma_Hdr.Id='" + hdnID.Value + "'", con);
        da.Fill(Dt_Product);
    ...
    private void SaveRecord()
    {
        ...
        {
            SqlCommand Cmd = new SqlCommand("UPDATE tbl_Proforma_Hdr SET InvoiceNo=@InvoiceNo,InvoiceDate=@InvoiceDate,PoNo=@PoNo,PoDate=@PoDate," +
            ...
            SqlDataAdapter daa = new SqlDataAdapter("SELECT JobNo,Description,Hsn,TaxPercentage,Quntity,Unit,Rate," +
                "DiscountPercentage,Total FROM [tbl_Proforma_Dtls] WHERE Id='" + hdnID.Value + "'", con);
            daa.Fill(Dt);

            SqlCommand CmdDelete = new SqlCommand("DELETE FROM [tbl_Proforma_Dtls] WHERE InvoiceId=@InvoiceId", con);
            ...
            foreach (GridViewRow g1 in gvPurchaseRecord.Rows)
            {
                ...
                SqlCommand Cmd2 = new SqlCommand("INSERT INTO [tbl_Proforma_Dtls] (InvoiceId,Hsn,TaxPercentage,Quntity,Unit,Rate,DiscountPercentage, MateName, PrintDescription,Total) " +
        ...
        else
        {
            ...
            try
            {
                ...
                SqlCommand Cmd = new SqlCommand("SP_Proforma_new", con);
                ...
                foreach (GridViewRow G1 in gvPurchaseRecord.Rows)
                {
                    ...
                    string MateName = (G1.FindControl("lblproduct") as Label).Text;
                    string PrintDescription = (G1.FindControl("LblPrintdescription") as Label).Text;
                    SqlCommand Cmd1 = new SqlCommand("INSERT INTO [tbl_Proforma_Dtls] (InvoiceId,Hsn,TaxPercentage,Quntity,Unit,Rate,DiscountPercentage,Total,MateName,PrintDescription ,InvoiceNo) " +
                ...
                ClientScript.RegisterStartupScript(this.GetType(), "alert", "HideLabel('Data saved Sucessfully');window.location ='/Admin/SalesProformaList.aspx';", true);
            }
            catch (Exception ex)
            {
                throw ex;
    ...
    private MemoryStream PDF(string InvoiceNo, string message)
    {
        ...
        //    {
                ...
                SqlDataAdapter Da = new SqlDataAdapter("SELECT * FROM vw_Taxinvoice_pdf As TP Inner join [tbl_Proforma_Dtls] As TD on TP.Id= TD.InvoiceId WHERE TP.InvoiceNo='" + txt_InvoiceNo.Text + "' ", con);
                ...
                if (Dt.Rows.Count > 0)
                {
                    ...
                    table.AddCell(new Phrase("Shipinig Address : ", FontFactory.GetFont("Arial", 9, Font.BOLD)));
                    table.AddCell(new Phrase(CompanyAddress, FontFactory.GetFont("Arial", 9, Font.BOLD)));

                    table.AddCell(new Phrase("Biling Address : ", FontFactory.GetFont("Arial", 9, Font.BOLD)));
                    ...
                    cn.Rectangle(17f, 15f, 560f, 135f);
                    cn.Stroke();
                    ...
                    cn.SetFontAndSize(BaseFont.CreateFont(@"C:\Windows\Fonts\Calibrib.ttf", "Identity-H", BaseFont.EMBEDDED), 14);
                    cn.ShowTextAligned(PdfContentByte.ALIGN_LEFT, "Bank Details :", 30, 83, 0);
                    cn.SetFontAndSize(BaseFont.CreateFont(@"C:\Windows\Fonts\calibri.ttf", "Identity-H", BaseFont.EMBEDDED), 12);
                    cn.ShowTextAligned(PdfContentByte.ALIGN_LEFT, "Bank Name : BANK OF BARODA", 30, 69, 0);
                    cn.SetFontAndSize(BaseFont.CreateFont(@"C:\Windows\Fonts\calibri.ttf", "Identity-H", BaseFont.EMBEDDED), 12);
                    cn.ShowTextAligned(PdfContentByte.ALIGN_LEFT, "Account Name : ENDEAVOUR AUTOMATION", 30, 57, 0);
                    cn.SetFontAndSize(BaseFont.CreateFont(@"C:\Windows\Fonts\calibri.ttf", "Identity-H", BaseFont.EMBEDDED), 12);
                    cn.ShowTextAligned(PdfContentByte.ALIGN_LEFT, "Branch : KALEWADI", 30, 45, 0);
                    cn.SetFontAndSize(BaseFont.CreateFont(@"C:\Windows\Fonts\calibri.ttf", "Identity-H", BaseFont.EMBEDDED), 12);
                    cn.ShowTextAligned(PdfContentByte.ALIGN_LEFT, "A/c No : 46180200000214", 30, 33, 0);
                    cn.SetFontAndSize(BaseFont.CreateFont(@"C:\Windows\Fonts\calibri.ttf", "Identity-H", BaseFont.EMBEDDED), 12);
                    cn.ShowTextAligned(PdfContentByte.ALIGN_LEFT, "IFSC/Neft Code :BARB0KALEWA", 30, 20, 0);
    ...
    {
        SqlDataAdapter Da = new SqlDataAdapter("SELECT * FROM [tblCustomer] WHERE CustomerName='" + txtCompName.Text + "'  ", con);
        DataTable Dt = new DataTable();
        Da.Fill(Dt);
        if (Dt.Rows.Count > 0)
        {
