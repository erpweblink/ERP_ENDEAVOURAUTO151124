...
public partial class Admin_CustomerPO_List_Sales : System.Web.UI.Page
{
    ...
    void gvbind_Company()
    {
        try
        {
            ...
            SqlDataAdapter sad = new SqlDataAdapter("SELECT Id,CustomerName,SubCustomer,JobNo,Pono,PoDate,RefNo,Mobileno,GrandTotal,CreatedBy,CreatedOn,DATEDIFF(DAY, PoDate, getdate()) AS days FROM CustomerPO_Hdr_Sales WHERE Is_Deleted='0' AND CustomerName='" + UserCompany + "'", con);
            sad.Fill(dt);
    ...
    private void ViewData()
    {
        ...
        DataTable Dt = new DataTable();
        SqlDataAdapter da = new SqlDataAdapter("SELECT Id,jobNo,Quotationno,CustomerName,SubCustomer,Pono,PoDate,RefNo,Mobileno,GrandTotal,CreatedBy,CreatedOn,DATEDIFF(DAY, PoDate, getdate()) AS days FROM CustomerPO_Hdr_Sales WHERE Is_Deleted='0' ORDER BY CreatedOn DESC", con);
        da.Fill(Dt);
        GvCustomerpoList.EmptyDataText = "Records Not Found";
        GvCustomerpoList.DataSource = Dt;
        GvCustomerpoList.DataBind();
