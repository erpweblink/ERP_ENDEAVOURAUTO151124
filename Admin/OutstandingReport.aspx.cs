...
public partial class Admin_OutstandingReport : System.Web.UI.Page
{
    ...
    void OutstandingGridView()
    {
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                Dtt = GetData("SP_OutstandingReportTest", dt.Rows[0]["CustName"].ToString(), fdate, tdate);
                GVOutstandingRpt.EmptyDataText = "Not Records Found";
                GVOutstandingRpt.DataSource = Dtt;
                GVOutstandingRpt.DataBind();
            }
            con.Close();
    ...
    public static List<string> AutoFillCust(string prefixText)
    {
        using (SqlConnection con = new SqlConnection())
        {
            ...
            {
                com.CommandText = "select DISTINCT CustName from tblInvoiceHdr" +
    ...
    private static DataTable GetData(string SP, string PartyName, string FromDate, string ToDate)
    {
        ...
        using (SqlConnection con = new SqlConnection(strConnString))
        {
            using (SqlCommand cmd = new SqlCommand(SP, con))
            {
                ...
                if (FromDate =="" && ToDate=="")
                {
                 cmd.Parameters.AddWithValue("@Type", "Getcustomerwise");
                }

                else
                {
                    cmd.Parameters.AddWithValue("@Type", "GetcustomerDatewise");
                }
                ...
                else
                {
                    ...
                    DateTime parsedLastRansaction;
                    if (DateTime.TryParse(Fdate, out parsedLastRansaction))
                    {
                        string formattedDate1 = parsedLastRansaction.ToString("yyyy-MM-dd"); ;

                        cmd.Parameters.AddWithValue("@FromDate", formattedDate1);
                    }
                ...
                else
                {
                    string Tdate = ToDate;
                    DateTime parsedLastRansaction;
                    if (DateTime.TryParse(Tdate, out parsedLastRansaction))
                    {
                        string ToDate1 = parsedLastRansaction.ToString("yyyy-MM-dd"); ;
    ...
    private void Pdf()
    {
        ...
        {
            Da = new SqlDataAdapter("select distinct(CustName),CustomerShippingAddress from tblInvoiceHdr where CustName='" + txtcustomername.Text + "'", con);

            // Da = new SqlDataAdapter("SELECT * FROM VW_Outstanding WHERE CustName='" + txtcustomername.Text + "'", con);
        }
    ...
    private void Export_Excel()
    {
        try
        {
            ...
            foreach (GridViewRow row in GVOutstandingRpt.Rows)
            {
    ...
    private void ExportGridToExcel()
    {
        ...
        Response.Charset = "";
        string FileName = "Outstanding Reports" + DateTime.Now + ".xls";
        StringWriter strwritter = new StringWriter();
        HtmlTextWriter htmltextwrtter = new HtmlTextWriter(strwritter);
        Response.Cache.SetCacheability(HttpCacheability.NoCache);
        Response.ContentType = "application/vnd.ms-excel";
        Response.AddHeader("Content-Disposition", "attachment;filename=" + FileName);
        GvExcell.GridLines = GridLines.Both;
        GvExcell.HeaderStyle.Font.Bold = true;
        GvExcell.RenderControl(htmltextwrtter);
    ...
    protected void btnDownloadExcel_Click1(object sender, EventArgs e)
    {
        OutstandingExcellGridView();
    ...
    private void Export_Excel1()
    {
        try
        {
            ...
            // Iterate through each row in the GridView and populate the DataTable
            foreach (GridViewRow row in GVOutstandingRpt.Rows)
            {
                string Invoiceno = (row.Cells[1].FindControl("lblinvoiceno") as Label)?.Text;
                string Invoicedate = (row.Cells[1].FindControl("lblinvoicedate") as Label)?.Text;
            ...
            Response.Clear();
            Response.Buffer = true;
            Response.AddHeader("content-disposition", "attachment;filename=OutstandingGridView.xls");
            Response.Charset = "";
            Response.ContentType = "application/vnd.ms-excel";
            ...
            using (StringWriter sw = new StringWriter())
            {
                using (HtmlTextWriter hw = new HtmlTextWriter(sw))
                {
                    ...
                    // Write the style and content to the response
   ...
   public  void OutstandingExcellGridView()
    {
        try
        {
            DataTable dt = new DataTable();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                Dtt = GetData("SP_OutstandingReportTest", dt.Rows[0]["CustName"].ToString(), fdate, tdate);
                GvExcell.EmptyDataText = "Not Records Found";
                GvExcell.DataSource = Dtt;
                GvExcell.DataBind();
            }
            con.Close();
