...
public partial class Admin_DebitNote : System.Web.UI.Page
{
    ...
    protected void Loaddate(string id)
    {
        DataTable dt1 = new DataTable();
        SqlDataAdapter sad1 = new SqlDataAdapter("select * from tblDebitNotePurchaseHdrs where Voc_no='"+ ID + "'", con);
        sad1.Fill(dt1);
        if(dt1.Rows.Count>0)
        {
            txtVocNo.Text = dt1.Rows[0]["Voc_no"].ToString();
           txtrefNo.Text = dt1.Rows[0]["Ref_No"].ToString();
            txtorderno.Text = dt1.Rows[0]["OrderNo"].ToString();
            txtJobNo.Text = dt1.Rows[0]["JobNo"].ToString();
            txtnarration.Text = dt1.Rows[0]["Narration"].ToString();
            ddldropdownlist.Text = dt1.Rows[0]["Type"].ToString();
            txtCompName.Text = dt1.Rows[0]["CustomerName"].ToString();
            txt_CompanyGSTno.Text = dt1.Rows[0]["GstNo"].ToString();
            txtStateCode.Text = dt1.Rows[0]["statecode"].ToString();
            txt_CompanyAddress.Text = dt1.Rows[0]["Address"].ToString();
            txtmobileno.Text = dt1.Rows[0]["MobileNo"].ToString();
            txt_cgst_amt.Text = dt1.Rows[0]["CGST"].ToString();
            txt_sgst_amt.Text = dt1.Rows[0]["SGST"].ToString();
            txt_igst_amt.Text = dt1.Rows[0]["IGST"].ToString();
            txt_grand_total.Text = dt1.Rows[0]["GrandTotal"].ToString();
            lbl_Amount_In_Word.Text = dt1.Rows[0]["InwordAmt"].ToString();
            DateTime ffff2 = Convert.ToDateTime(dt1.Rows[0]["VocDate"].ToString());
            txtvochardate.Text = ffff2.ToString("yyyy-MM-dd");
            DateTime ffff3 = Convert.ToDateTime(dt1.Rows[0]["orderDate"].ToString());
        ...
        SqlDataAdapter daa = new SqlDataAdapter("SELECT  Description,Hsn,Tax,Quntity,Unit,Rate,Discount,TotalAmt FROM tblDebitNotePurchaseDtls WHERE Voc_no='" + txtVocNo.Text + "'", con);
        daa.Fill(Dt_Product);
        ...
        gvDebitnote.DataSource = Dt_Items;
        gvDebitnote.DataBind();

        ///load email 
        SqlDataAdapter Sda = new SqlDataAdapter("SELECT * FROM tblDebitNoteEmail WHERE Voc_no='" + txtVocNo.Text + "'", con);
    ...
    {
        SqlDataAdapter ad = new SqlDataAdapter("SELECT max([Id]) as maxid FROM [tblDebitNotePurchaseHdrs]", con);
    ...
    protected void txtCompName_TextChanged(object sender, EventArgs e)
    {
        ...
        ///load mail in vendor table
        DataTable dt1 = new DataTable();
        SqlDataAdapter sad1 = new SqlDataAdapter("select * from tblVendorContact where vendorName='" + txtCompName.Text + "'", con);
        sad1.Fill(dt1);
        Grd_MAIL.DataSource = dt1;
        Grd_MAIL.DataBind();
    ...
    private void ShowGrid()
    {
        ...
        DataTable Dt = (DataTable)ViewState["DebitNote"];
    ...
    protected void taxcalculation()
    {
        ...
        foreach (GridViewRow g1 in gvDebitnote.Rows)
        {
    ...
    protected void gvDebitnote_RowEditing(object sender, GridViewEditEventArgs e)
    {
        gvDebitnote.EditIndex = e.NewEditIndex;
        gvDebitnote.DataSource = (DataTable)ViewState["DebitNote"];
        gvDebitnote.DataBind();
        ScriptManager.RegisterStartupScript(this, this.GetType(), "Success", "scrollToElement();", true);
    }
    ...
    protected void gv_update_Click(object sender, EventArgs e)
    {
        ...
        DataTable Dt = ViewState["DebitNote"] as DataTable;
    ...
    decimal Alltotal;
    protected void gvDebitnote_RowDataBound(object sender, GridViewRowEventArgs e)
    {
    ...
    protected void saverecord()
    {
        ...
        try
        {
            ...
            {
                SqlCommand cmd = new SqlCommand("SP_DebitNoteHdrs", con);
                ...
                cmd.Parameters.AddWithValue("@InwordAmt", lbl_Amount_In_Word.Text);
                ...
                foreach (GridViewRow G1 in gvDebitnote.Rows)
                {
                    ...
                    SqlCommand Cmd1 = new SqlCommand("SP_DebitNoteDtls", con);
                ...
                foreach (GridViewRow g1 in Grd_MAIL.Rows)
                {
                    ...
                    SqlCommand cmdtable = new SqlCommand("insert into tblDebitNoteEmail(Voc_no,Email) values(@Voc_no,@Email)", con);
           ...
           else if (btn_save.Text == "Update")
            {
                ...
                SqlDataAdapter sada = new SqlDataAdapter("select * from tblDebitNotePurchaseDtls where Voc_no='" + txtVocNo.Text + "'", con);
                sada.Fill(dtdetails);
                if (dtdetails.Rows.Count == gvDebitnote.Rows.Count)
                {
                    ////update description in details tables
                    foreach (GridViewRow G1 in gvDebitnote.Rows)
                    {
                        string Discription = (G1.FindControl("txt_discription_grd") as Label).Text;
                        string HSN = (G1.FindControl("txt_hsn_grd") as Label).Text;
                ...
                {
                    SqlCommand CmdDelete = new SqlCommand("DELETE FROM tblDebitNotePurchaseDtls WHERE Voc_no=@Voc_no", con);
                    ...
                    ///email update
                    SqlDataAdapter Sda = new SqlDataAdapter("SELECT * FROM tblDebitNoteEmail WHERE VocNo='" + txtVocNo.Text + "'", con);
                    ...
                    foreach (GridViewRow g1 in Grd_MAIL.Rows)
                    {
                        ...
                        SqlCommand cmdtable = new SqlCommand("UPDATE tblDebitNoteEmail SET Voc_no=@Voc_no,Email=@Email WHERE Voc_no=@Voc_no AND Email=@Email", con);
                        cmdtable.Parameters.AddWithValue("@Voc_no", txtVocNo.Text);
                        cmdtable.Parameters.AddWithValue("@Email", MAIL);
                        // cmdtable.Parameters.AddWithValue("@chkMail", chkmail);
                        con.Open();
