...
public partial class reportPdf_ReceiptVoucherPdf : System.Web.UI.Page
{
    string id = "";
    ...
    private void Pdf(string id)
    {
       ...
       // SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM [TblReceiptVoucherHdr] where Id='" + id + "'", Conn);
        // SqlDataAdapter da = new SqlDataAdapter("SELECT tblPurcahseInvoiceHdr.Id,VendorName,Voc_No,VocDate,MobileNo,KindAtt,Address,GstNo,CGST,SGST,IGST,GradTotal, TermCondition1, TermCondition2, TermCondition3, TermCondition4, Createddate from tblPurcahseInvoiceHdr INNER JOIN TblPaymentVoucherDtls ON tblPurcahseInvoiceHdr.Id = TblPaymentVoucherDtls.Header_Id WHERE tblPurcahseInvoiceHdr.Id='" + id + "'", Conn);
        ...
        PdfWriter writer = PdfWriter.GetInstance(doc, new FileStream(Server.MapPath("~/Files/") + "ReceiptVoucher.pdf", FileMode.Create));
        ...
        cd.ShowTextAligned(PdfContentByte.ALIGN_LEFT, "RECEIPT VOUCHER", 225, 687, 0);
        cd.EndText();
        ...
        if (Dt.Rows.Count > 0)
        {
            ...
            string ReceiptNo = Dt.Rows[0]["ReceiptNo"].ToString();
            string Date = Dt.Rows[0]["Date"].ToString().TrimEnd("0:0".ToCharArray());
            string Address = Dt.Rows[0]["Address"].ToString();
            string PaymentMode = Dt.Rows[0]["PaymentMode"].ToString();
            string ChequeTransNo = Dt.Rows[0]["ChequeTransIdNo"].ToString();
            string DrownOnBankCash = Dt.Rows[0]["BankCashDr"].ToString();
            string CreatedDate = Dt.Rows[0]["CreatedDate"].ToString().TrimEnd("0:0".ToCharArray());
            string TotalAmount = Dt.Rows[0]["Balance"].ToString();
            ...
            table.AddCell(new Phrase("Cust Name : ", FontFactory.GetFont("Arial", 9, Font.BOLD)));
            ...
            table.AddCell(new Phrase(ReceiptNo, FontFactory.GetFont("Arial", 9, Font.BOLD)));
            ...
            if (Dt.Rows.Count > 0)
            {
                ...
                foreach (DataRow dr in Dt.Rows)
                {
                    ...
                    table.AddCell(new Phrase(dr["ReceiptNo"].ToString(), FontFactory.GetFont("Arial", 9)));
                    table.AddCell(new Phrase(dr["Date"].ToString(), FontFactory.GetFont("Arial", 9)));
                    table.AddCell(new Phrase(dr["Balance"].ToString(), FontFactory.GetFont("Arial", 9)));
            ...
            PdfPCell cell77 = new PdfPCell(new Phrase(TotalAmount, FontFactory.GetFont("Arial", 10, Font.BOLD)));
        ...
        doc.Close();
        ifrRight6.Attributes["src"] = @"../Files/" + "ReceiptVoucher.pdf";
    }
}