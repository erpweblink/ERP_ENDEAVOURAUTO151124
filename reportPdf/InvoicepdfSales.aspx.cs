...
public partial class Invoicepdf : System.Web.UI.Page
{
    ...
    private void Pdf(string flg)
    {
        ...
        if (Dt.Rows.Count > 0)
        {
            ...
            //table.AddCell(paragraph);
            //PdfPCell cell666 = new PdfPCell(new Phrase("I / we certify that our registration certficate under the GST Act, 2017 is in force on the date on which the supply of goods specified In This Tax Invoice.", FontFactory.GetFont("Arial", 8)));
            //cell666.HorizontalAlignment = Element.ALIGN_CENTER;
            //table.AddCell(cell666);
            //PdfPCell cell667 = new PdfPCell(new Phrase("is made by me / us & the transaction of supply covered by this Tax Invoice had been effected by me / us & it shall be accounted for in the turnover of", FontFactory.GetFont("Arial", 8)));
            //cell667.HorizontalAlignment = Element.ALIGN_CENTER;
            //table.AddCell(cell667);
            //PdfPCell cell668 = new PdfPCell(new Phrase("supplies while filling of return &the due tax if any payable on the supplies has been paid or shall be paid. Further certified that the particulars given", FontFactory.GetFont("Arial", 8)));
            //cell668.HorizontalAlignment = Element.ALIGN_CENTER;
            //table.AddCell(cell668);
            //PdfPCell cell669 = new PdfPCell(new Phrase("above are true and correct & the amount indicated represents the prices actually charged and that there is no flow additional consideration directly or", FontFactory.GetFont("Arial", 8)));
            //cell669.HorizontalAlignment = Element.ALIGN_CENTER;
            //table.AddCell(cell669);
            //PdfPCell cell6699 = new PdfPCell(new Phrase("indirectly from the buyer. Interest @ 18% p.a. charged on all outstanding more than one month after invoice has been rendered", FontFactory.GetFont("Arial", 8)));
            //cell6699.HorizontalAlignment = Element.ALIGN_CENTER;
            //table.AddCell(cell6699);

            //doc.Add(table);

            //new code end

            //new code for terms Start



            //new code for terms End
            ...
            cn.ShowTextAligned(PdfContentByte.ALIGN_LEFT, " Terms and Conditions  ", 30, 125, 0);
