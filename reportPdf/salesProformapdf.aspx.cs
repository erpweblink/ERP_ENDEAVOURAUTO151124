...
public partial class reportPdf_salesProforma : System.Web.UI.Page
{
    ...
    private void Pdf(string flg)
    {
        ...
        SqlDataAdapter da = new SqlDataAdapter("SELECT [tbl_Proforma_Hdr].Id,[tbl_Proforma_Hdr].InvoiceNo,InvoiceDate,PoNo,PoDate,CustName,ChallanNo,ChallanDate,PayTerm," +
            ...
            " Total,CreatedOn FROM [tbl_Proforma_Hdr] INNER JOIN [tbl_Proforma_Dtls] ON [tbl_Proforma_Dtls].InvoiceId = [tbl_Proforma_Hdr].Id WHERE [tbl_Proforma_Hdr].Id='" + id + "'", Conn);
        da.Fill(Dt);
        ...
        cdd.ShowTextAligned(PdfContentByte.ALIGN_LEFT, "Proforma", 260, 667, 0);
        cdd.EndText();
        //--------------
        ...
        if (Dt.Rows.Count > 0)
        {
            ...
            table.AddCell(new Phrase("Proforma Number : ", FontFactory.GetFont("Arial", 9, Font.BOLD)));
            table.AddCell(new Phrase(InvoiceNo, FontFactory.GetFont("Arial", 9, Font.BOLD)));

            table.AddCell(new Phrase("Proforma Date :", FontFactory.GetFont("Arial", 9, Font.BOLD)));
    ...
    //{
    //    DataTable Dt = new DataTable();
    //    SqlDataAdapter da = new SqlDataAdapter("select * from vw_SalesProformaPdfreport where Id='" + id + "'", Conn);
    //    da.Fill(Dt);
    //    //GvPurchaseOrderList.DataSource = Dt;
    //    // GvPurchaseOrderList.DataBind();
    ...
    //        var Grandtotal = Convert.ToDecimal(Ttotal_price) + Convert.ToDecimal(SGST) + Convert.ToDecimal(SGST);
    ...
    //        Byte[] FileBuffer = File.ReadAllBytes(Server.MapPath("~/Files/") + "ProformaInvoice.pdf");
    ...
    //    }
    //    doc.Close();
    //    ifrRight6.Attributes["src"] = @"../Files/" + "ProformaInvoice.pdf";
    //} 
